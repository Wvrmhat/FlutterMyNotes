...
void main() {
  ...
  group('Mock Authentication', () {           // tests can be grouped which can be run entirely
    final provider = MockAuthProvider();          // creates instance of our mock auth provider
    test('Should not be initialized to begin with', () {
      expect(provider.isInitialized, false);          // if it returns true then test will fail
    });

    test('Cannot log out if not initialized', () {
      expect(
        provider.logOut(),                                          // execute logout function and test result of that function in the matcher
        throwsA(const TypeMatcher<NotInitializedException>()),    // throwsA is a matcher, that matches the lresult of the logout function
      );
    });
...
class MockAuthProvider implements AuthProvider {      // mock auth code with its own initializers
  AuthUser? _user;
